...
class apiQuery:
    ...
    def queryGPT(self, messages):
        ...
        res = response["choices"][0]["message"][
            "content"
        ]  # Selecting first choice's content
        return res

    # Function to query weather network API for forecast data
    # API for endpoint coordinates, we are using Austin TX
    ...
    def queryForecast(self, intent, day):
        try:
            ...
            georeq = f'http://api.openweathermap.org/geo/1.0/direct?q=Austin,TX,US&appid={os.getenv("WEATHER_KEY")}'
            ...
            if intent == "uviForecast":  # <-- If intent is uviForecast
                for daily_data in responsejson[
                    "daily"
                ]:  # <-- Iterate over daily forecasts and find matching timestamp to the day parameter
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        "%Y-%m-%d"
                    ) == day.strftime("%Y-%m-%d"):
                        uv_index = daily_data[
                            "uvi"
                        ]  # <-- If day matches, set uv_index to the uvi value
                ...
                for daily_data in responsejson[
                    ...
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        ...
                        wind_speed = daily_data[
                            "wind_speed"
                        ]  # <-- If day matches, set wind speed to the wind_speed value
                ...
                for daily_data in responsejson[
                    ...
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        ...
                        temp = daily_data["temp"][
                            "max"
                        ]  # <-- If day matches, set temp to the max temp value
                ...
                for daily_data in responsejson[
                    ...
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        ...
                        temp = daily_data["temp"][
                            "min"
                        ]  # <-- If day matches, set temp to the min temp value
                ...
                for daily_data in responsejson[
                    ...
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        ...
                        avg_temp = daily_data["temp"][
                            "day"
                        ]  # <-- If day matches, set temp to the avg temp value
                ...
                for daily_data in responsejson[
                    ...
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        ...
                        dew_point = daily_data[
                            "dew_point"
                        ]  # <-- If day matches, set dew point
                        # Create data dictionary with the day, date, and low temp
                        data = {
                            "day": datetime.datetime.strftime(day, "%A"),
                            "date": day,
                ...
                for daily_data in responsejson[
                    ...
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        ...
                        humidity = daily_data[
                            "humidity"
                        ]  # <-- If day matches, set humidity to the humidity value
                ...
                for daily_data in responsejson[
                    ...
                    if datetime.datetime.utcfromtimestamp(daily_data["dt"]).strftime(
                        ...
                        pressure = daily_data[
                            "pressure"
                        ]  # <-- If day matches, set pressure to the pressure value ( returns in hPa )
    ...
    def queryWeather(self, intent):
        ...
        if intent == "currentTemperature":  # <-- If intent is currentTemperature
            ...
            data = {"current_temp": current_temp, "feels_like": feels_like}
            ...
            data = next(
                (
                    weather["description"]
                    for weather in responsejson["current"]["weather"]
                    if weather["main"] == "Rain"
                ),
                None,
            )
            ...
            timestamp0 = datetime.datetime.fromtimestamp(
                day0
            )  # Convert Unix timestamp to Human-Readable datetime
            ...
            data = {"today_sunrise": sunrise_0, "tomorrow_sunrise": sunrise_1}
        ...
        # FORECASTING: First check if it will rain today, then check for rain in the next 7 days.
            ...
            for forecast in responsejson["hourly"]:  # <-- Loop through hourly forecast
                dt = datetime.datetime.fromtimestamp(
                    forecast["dt"]
                )  # <-- Convert Unix timestamp to datetime for index
                if (
                    "rain" in forecast
                    and forecast["rain"]["1h"] > 0
                    and dt.date() == now.date()
                ):  # <-- If it will rain today
                    rain_pop = forecast["pop"]  # <-- Get probability of precipitation
                    rain_dt = datetime.datetime.fromtimestamp(
                        forecast["dt"]
                    )  # <-- Convert Unix timestamp to datetime
                    rain_time = rain_dt.strftime(
                        "%I:%M %p %Z"
                    ).strip()  # <-- Format datetime to hour:minute AM/PM

                    rain_times.append(
                        (rain_time, rain_pop)
                    )  # <-- Append time and probability of precipitation to list

            if (
                rain_times
            ):  # <-- If list is not empty store rain times in dictionary at index 'Today'
                days_with_rain["Today"] = rain_times

            # NEXT 7 DAYS FORECAST
            for forecast in responsejson["daily"][
                1:8
                ...
                if (
                    "rain" in forecast and forecast["rain"] > 0
                ):  # <-- If it will rain on indexed day
                    days_with_rain[dt.strftime("%A")] = forecast[
                        "rain"
                    ]  # <-- Store rain amount in dictionary at index of day
